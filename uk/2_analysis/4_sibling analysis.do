/*******************************************************************************

	Generate table showing the sibling analysis models of antidepressant use during pregnancy and the outcomes of interest

	Author: Paul Madley-Dowd (amended by Flo Martin)

	Date: 02/07/2024

	Table generated by this script
	
		- sibling analysis.txt sibling analysis conditional logisitic regression models for each outcome
		
*******************************************************************************/

* Start logging

	log using "$Logdir\2_analysis\4_sibling analysis", name(sibling_analysis) replace
	
********************************************************************************
	
	ssc install tabcount
	
* Load in the analysis dataset
	
	use "$Datadir\primary_analysis_dataset.dta", clear
	
	rename CPRD_consultation_events_cat healthcare_util_12mo_cat
	
	* Sibling analysis data management
	
	preserve
	
		sort patid pregid pregstart_num
		
		duplicates drop patid pregid, force // checking that there're no multiple pregnancies still in 
		
		by patid: egen _seq = seq()
		
		keep patid _seq any_preg
		
		reshape wide any_preg, i(patid) j(_seq)
		
		egen famavg_any_preg = rowmean(any_preg*) 
		
		keep patid famavg_any_preg
		
		save "$Tempdatadir\famavg_exposure.dta", replace
		
	restore 
	
	merge m:1 patid using "$Tempdatadir\famavg_exposure.dta", nogen
	
	* deviation from family averaged covariate 
	
	foreach covar in birth_yr mother_age_cat smoke_preg grav_hist_sb imd_practice parity ap_12mo asm_12mo healthcare_util_12mo_cat depression anxiety {
		
		preserve 
			
			sort patid pregid pregstart_num 
			duplicates drop patid pregid , force 
			by patid: egen _seq = seq() // generate identifier for each pregnacy a mother has - 11 babies maximum
			keep patid _seq `covar'	
			reshape wide `covar', i(patid) j(_seq)
			egen fa_`covar' = rowmean(`covar'*)  
			keep patid fa_`covar'
			save "$Tempdatadir\famavg_`covar'.dta", replace
		
		restore	
	
	}
	
	foreach covar in birth_yr mother_age_cat mother_ethn smoke_preg grav_hist_sb imd_practice parity ap_12mo asm_12mo healthcare_util_12mo_cat depression anxiety {	
		
		merge m:1 patid using "$Tempdatadir\famavg_`covar'.dta", nogen
		gen dev_`covar' = `covar'-fa_`covar'	
		
	}
	
	* Identify family size 
	
	sort patid pregstart_num
	by patid: egen border = seq()
	by patid: egen famsize = max(border)
	
	preserve 
		
		keep if border <= 2 & famsize >=2
		keep patid border *any_preg
		drop famavg*
		reshape wide any_preg, i(patid) j(border) 

		gen firstsibexp_any1  = 1 if  any_preg1 == 1 & any_preg2 == 0 
		gen firstsibexp_any2  = firstsibexp_any1	
			
		gen secondsibexp_any1 = 1 if  any_preg1 == 0 & any_preg2 == 1 	
		gen secondsibexp_any2 = secondsibexp_any1		
	
		keep patid firstsibexp_* secondsibexp_*
		 
		reshape long firstsibexp_any secondsibexp_any, i(patid) j(border) 
		 
		save "$Tempdatadir\ord_exposure_expdisc", replace 
		 
	restore 
	
	merge m:1 patid border using "$Tempdatadir\ord_exposure_expdisc", nogen
	
	*drop if child_male==.
	
	* Family averaged covariates for shared covariates
	
	foreach covar in birth_yr ap_12mo asm_12mo healthcare_util_12mo_cat depression anxiety smoke_preg grav_hist_sb {
	    
		gen `covar'_model = fa_`covar' - `covar'
		
	}
	
	* Preterm exposure - dropping new users after 36+6 weeks' gestation
	
	gen any_preg_preterm = 0 if any_preg==0 | flag_37wk_initiators==1
	replace any_preg_preterm = 1 if any_preg_preterm==.
	
	save "$Deriveddir\sibling_analysis.dta", replace
	
* Sibling analysis
	
	tempname myhandle	
	file open `myhandle' using "$Tabledir\sibling analysis.txt", write replace
	
	file write `myhandle' "Outcome" _tab "Total" _tab "Exposed n/N" _tab "Unexposed n/N" _tab "OR" _tab "aOR*" _n
	
	foreach outcome in stillborn preterm postterm sga_pct lga_pct {
		
		use "$Deriveddir\sibling_analysis.dta", clear
	
		gen cc = 1 if birth_yr_cat!=. & mother_age_cat!=. & mother_ethn!=. & imd_practice!=. & parity!=. & ap_12mo!=. & asm_12mo!=. & depression!=. & anxiety!=. & healthcare_util_12mo_cat!=. & smoke_preg!=.
		keep if cc==1
	    
		* Use the preterm exposure variable for the preterm models (no new users after 37 weeks' in the exposed group)
		local y="`outcome'"
		if "`y'"=="preterm" {
		    
			capture noisily clogit `outcome' i.any_preg_preterm, group(patid) or
		
			tabcount any_preg_preterm `outcome' if e(sample), v1(0/1) v2(0/1) matrix(exposed)
			local total=`e(N)'
			local unexp_lb=exposed[1,1]
			local unexp_sb=exposed[1,2]
			local unexp_n=`unexp_lb'+`unexp_sb'
			local exp_lb=exposed[2,1]
			local exp_sb=exposed[2,2]
			local exp_n=`exp_lb'+`exp_sb'
			
			disp `unexp_sb' 
			
			file write `myhandle' "`outcome'" _tab %7.0fc (`total') _tab %6.0fc (`exp_sb') ("/") %6.0fc (`exp_n') _tab %6.0fc (`unexp_sb') ("/") %6.0fc (`unexp_n')
			
			lincom 1.any_preg_preterm, or
			local minadjor=`r(estimate)'
			local minadjuci=`r(ub)'
			local minadjlci=`r(lb)'
			
			file write `myhandle' _tab %4.2f (`minadjor') (" (") %4.2f (`minadjlci') ("-") %4.2f (`minadjuci') (")")
			
			capture noisily clogit `outcome' i.any_preg_preterm birth_yr_model smoke_preg_model i.mother_age_cat i.mother_ethn i.imd_practice i.parity ap_12mo_model asm_12mo_model healthcare_util_12mo_cat_model depression_model anxiety_model, group(patid) or
			
			lincom 1.any_preg_preterm, or
			local minadjor=`r(estimate)'
			local minadjuci=`r(ub)'
			local minadjlci=`r(lb)'
			
			file write `myhandle' _tab %4.2f (`minadjor') (" (") %4.2f (`minadjlci') ("-") %4.2f (`minadjuci') (")") _n
			
		}
		
		else {
		    
			capture noisily clogit `outcome' i.any_preg, group(patid) or
		
			tabcount any_preg `outcome' if e(sample), v1(0/1) v2(0/1) matrix(exposed)
			
			local total=`e(N)'
			local unexp_lb=exposed[1,1]
			local unexp_sb=exposed[1,2]
			local unexp_n=`unexp_lb'+`unexp_sb'
			local exp_lb=exposed[2,1]
			local exp_sb=exposed[2,2]
			local exp_n=`exp_lb'+`exp_sb'
			
			disp `unexp_sb' 
			
			file write `myhandle' "`outcome'" _tab %7.0fc (`total') _tab %6.0fc (`exp_sb') ("/") %6.0fc (`exp_n') _tab %6.0fc (`unexp_sb') ("/") %7.0fc (`unexp_n')
			
			lincom 1.any_preg, or
			local minadjor=`r(estimate)'
			local minadjuci=`r(ub)'
			local minadjlci=`r(lb)'
			
			file write `myhandle' _tab %4.2f (`minadjor') (" (") %4.2f (`minadjlci') ("-") %4.2f (`minadjuci') (")")
			
			capture noisily clogit `outcome' i.any_preg birth_yr_model i.mother_age_cat smoke_preg_model i.imd_practice i.mother_ethn i.parity ap_12mo_model asm_12mo_model healthcare_util_12mo_cat_model depression_model anxiety_model, group(patid) or
			
			lincom 1.any_preg, or
			local minadjor=`r(estimate)'
			local minadjuci=`r(ub)'
			local minadjlci=`r(lb)'
			
			file write `myhandle' _tab %4.2f (`minadjor') (" (") %4.2f (`minadjlci') ("-") %4.2f (`minadjuci') (")") _n
			
		}
	
	}

********************************************************************************

* Erase unnecessary datasets

	erase "$Tempdatadir\famavg_exposure.dta"
	
	foreach covar in birth_yr mother_age_cat smoke_preg grav_hist_sb imd_practice parity ap_12mo asm_12mo healthcare_util_12mo_cat depression anxiety {
	    
		erase "$Tempdatadir\famavg_`covar'.dta"
		
	}
	
	erase "$Tempdatadir\ord_exposure_expdisc.dta"

* Stop logging, translate .smcl into .pdf and erase .smcl

	log close sibling_analysis
	
	translate "$Logdir\2_analysis\4_sibling analysis.smcl" "$Logdir\2_analysis\4_sibling analysis.pdf", replace
	
	erase "$Logdir\2_analysis\4_sibling analysis.smcl"
	
********************************************************************************
