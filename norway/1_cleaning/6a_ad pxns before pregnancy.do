...
********************************************************************************
	...
	* Define periods of interest
	...
	keep mother_id preg_id start_date deliv_date secondtrim thirdtrim pregnum prepreg_*
	...
	* Merge on to prescription information and define exposure information for each pregnancy a mother has
	...
	bysort preg_id (mother_id): keep if pregnum==`x'
		...
		foreach y in l m n o { // pregnancy
			...
			merge 1:1 mother_id preg_id using "$Tempdatadir\prepreg_`x'_period`y'.dta", keep(1 3) 
	...
	* Merge onto full pregnancy cohort to check - all matched
	...
	keep mother_id preg_id pregnum *l *m *n *o
	order mother_id preg_id pregnum *l *m *n *o
...
* Save the dataset for patterns analysis
	...
	keep mother_id preg_id pregnum any_o any_n any_m any_l any_prepreg
