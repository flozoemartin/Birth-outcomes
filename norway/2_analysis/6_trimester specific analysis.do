/*******************************************************************************
	...
	Table generated by this script
		...
		- trimester specific data.txt unrounded estimates for figure creation
		
*******************************************************************************/

* Start logging

	log using "$Logdir\2_analysis\6_trimester specific analysis", name(trimester_specific_analysis) replace
		...
		* Recode incident users after 37 weeks' unexposed for the preterm delivery modesls
		...
		forvalues x=1/3 {
			...
			* Adjusted for all covariates
			...
			logistic `outcome' i.t`x' birth_yr i.mother_age_cat i.mother_educ i.mother_birth_country_nonnorge i.prev_sb_bin i.parity i.ap_12mo i.asm_12mo healthcare_util_12mo depression anxiety, vce(cluster mother_id) or
...
* FIGURE DATA
	...
	tempname myhandle	
	file open `myhandle' using "$Tabledir\trimester specific data.txt", write replace
	...
	foreach outcome in stillborn neonatal_death preterm postterm sga_pct lga_pct apgar5_bin { 
		...
		* Recode incident users after 37 weeks' unexposed for the preterm delivery modesls
		...
		forvalues x=1/3 {
			...
			file write `myhandle' "`outcome'" _tab "no" _tab (`x') _tab (`tot') _tab %6.0fc (`exp_n') ("/") %6.0fc (`exp') _tab %6.0fc (`unexp_n') ("/") %7.0fc (`unexp') _tab (`minadjor') _tab (`minadjlci') _tab (`minadjuci') _n
...
* Stop logging, translate .smcl into .pdf and erase .smcl
	...
	translate "$Logdir\2_analysis\6_trimester specific analysis.smcl" "$Logdir\2_analysis\6_trimester specific analysis.pdf", replace
	
	erase "$Logdir\2_analysis\6_trimester specific analysis.smcl"
