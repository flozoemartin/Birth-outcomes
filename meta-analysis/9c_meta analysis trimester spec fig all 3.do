...
* Make the maternal meta-analysis figure for supplementary material and generate the summary effect estimate for the primary figure
		...
		use "$Graphdir\data\trimester specific analyses data_all3.dta", clear
		...
		metan logor loglci loguci, eform by(outcome) lcols(outcome total_exp total_unexp) saving(trimesterspec_ma_`x', replace)
		
		* Use the data from metan to make a nicer forest plot
		
		use trimesterspec_ma_`x', clear
		...
		*drop if _USE==1
		...
		forestplot, eform lcols(total_exp total_unexp) effect("{bf}Odds ratio") null(1) olineopts(lcolor(none)) nlineopts(lcolor(cranberry) lpattern(dash)) xlabel(0.5(0.25)2.5) title("{bf}Antidepressant use in trimester `x' analysis", size(small)) ///
		...
		name(trimesterspec_ma_`x', replace)
		
		cd "$Graphdir"
		graph export trimesterspec_ma_`x'.pdf, replace 
	
	} 
	
	use trimesterspec_ma_1, clear
	gen trimester=1
	drop if _USE==0 | _USE==6
	
	append using trimesterspec_ma_2
	replace trimester=2 if trimester==.
	drop if _USE==0 | _USE==6
	
	append using trimesterspec_ma_3
	replace trimester=3 if trimester==.
	...
	foreach x in exp_y exp_total unexp_y unexp_total {
		...
		bysort _BY trimester: egen sum_`x'=total(`x')
		format sum_`x' %9.0fc
	...
	forvalues x=1/7 {
		forvalues y=1/3 {
			...
			sum sum_exp_y if _BY==`x' & trimester==`y'
			local sum_exp_y = `r(max)'
			local sum_exp_y_fc : display %6.0fc `sum_exp_y'
			
			sum sum_exp_total if _BY==`x' & trimester==`y'
			local sum_exp_total = `r(max)'
			local sum_exp_total_fc : display %7.0fc `sum_exp_total'
			
			sum sum_unexp_y if _BY==`x' & trimester==`y'
			local sum_unexp_y = `r(max)'
			local sum_unexp_y_fc : display %7.0fc `sum_unexp_y'
			
			sum sum_unexp_total if _BY==`x' & trimester==`y'
			local sum_unexp_total = `r(max)'
			local sum_unexp_total_fc : display %9.0fc `sum_unexp_total'
			
			replace total_exp = "`sum_exp_y_fc' / `sum_exp_total_fc'" if _BY==`x' & _USE==3  & trimester==`y'
			replace total_unexp = "`sum_unexp_y_fc' / `sum_unexp_total_fc'" if _BY==`x' & _USE==3  & trimester==`y'
	...
	replace _LABELS="Trimester 1" if trimester==1 & _USE==3
	replace _LABELS="Trimester 2" if trimester==2 & _USE==3
	replace _LABELS="Trimester 3" if trimester==3 & _USE==3
	...
	forestplot, eform lcols(total_exp total_unexp) effect("{bf}Odds ratio") null(1) olineopts(lcolor(none)) nlineopts(lcolor(cranberry) lpattern(dash)) xlabel(0.4(0.2)2 2.4 2.8, format(%3.1fc)) title("{bf}Trimester-specific analysis", size(small)) spacing(2) noadjust ///
		name(trimesterspec_ma, replace)
		
		cd "$Graphdir"
		graph export trimesterspec_ma.pdf, replace 

********************************************************************************

* Trimester one individual figure
		
	use trimesterspec_ma_1.dta, clear
	...
	* Macros to create the null line
	...
	title("{bf}Fixed-effect meta-analysis for antidepressant use during trimester one" "{bf}and birth outcomes in the UK, Norway, & Sweden", size(*0.5)) ///
	...
	xsize(90) ysize(100) name(trim1, replace)
	
	* Save graph
	
	graph export "C:\Users\ti19522\OneDrive - University of Bristol\Flo Martin Supervisory Team\Year 4\6_Birth outcomes\supplementary figures\trim1_ma.pdf", replace
	
********************************************************************************/

* Trimester two individual figure

	use trimesterspec_ma_2.dta, clear
	...
	* Macros to create the null line
	...
	xscale(range(0.1 5.7) log) xlab(0.4(0.2)2.9, labsize(*0.4) format(%3.1fc) angle(45))  ///
	graphregion(color(white) fcolor(white) ifcolor(white) lcolor(white)) plotregion(margin(1 1 0 1)) ///
	title("{bf}Fixed-effect meta-analysis for antidepressant use during trimester two" "{bf}and birth outcomes in the UK, Norway, & Sweden", size(*0.5)) ///
	text(-1 2.9 "{bf}aOR* (95% CI)", size(*0.35) justification(left) placement(e)) ///
	text(-1 4.4 "{bf}Weight (%)", size(*0.35) justification(left) placement(e)) ///
	text(-1 0.1 "{bf}Outcome" "	Country", size(*0.35) justification(left) placement(e)) ///
	text(-1 0.21 "{bf}Exposed" "{it:n} / N", size(*0.35) justification(right) placement(w)) ///
	...
	text(44 0.1 "*Adjusted for year of birth, maternal age," "previous stillbirth, parity, antipsychotic and" "anti-seizure medication use before pregnancy," "depression, anxiety, practice-level IMD and" "ethnicity (UK), maternal educational attainment" "and country of birth (Norway and Sweden)," "household disposable income (Sweden)," "smoking during pregnancy (UK and Sweden)," "maternal BMI (Sweden), number of primary care" "consultations before pregnancy (UK and Norway)", size(*0.35) justification(left) placement(e)) ///
	text(48 0.1 "{sup:‡}Norway and Sweden only", size(*0.35) justification(left) placement(e)) ///
	xsize(90) ysize(100) name(trim2, replace)
	
	* Save graph
	
	graph export "C:\Users\ti19522\OneDrive - University of Bristol\Flo Martin Supervisory Team\Year 4\6_Birth outcomes\supplementary figures\trim2_ma.pdf", replace
	
********************************************************************************

	use trimesterspec_ma_3.dta, clear
	...
	* Macros to create the null line
	...
	xscale(range(0.1 5.7) log) xlab(0.4(0.2)2.9, labsize(*0.4) format(%3.1fc) angle(45))  ///
	graphregion(color(white) fcolor(white) ifcolor(white) lcolor(white)) plotregion(margin(1 1 0 1)) ///
	title("{bf}Fixed-effect meta-analysis for antidepressant use during trimester three" "{bf}and birth outcomes in the UK, Norway, & Sweden", size(*0.5)) ///
	text(-1 2.9 "{bf}aOR* (95% CI)", size(*0.35) justification(left) placement(e)) ///
	text(-1 4.4 "{bf}Weight (%)", size(*0.35) justification(left) placement(e)) ///
	text(-1 0.1 "{bf}Outcome" "	Country", size(*0.35) justification(left) placement(e)) ///
	text(-1 0.21 "{bf}Exposed" "{it:n} / N", size(*0.35) justification(right) placement(w)) ///
	...
	text(44 0.1 "*Adjusted for year of birth, maternal age," "previous stillbirth, parity, antipsychotic and" "anti-seizure medication use before pregnancy," "depression, anxiety, practice-level IMD and" "ethnicity (UK), maternal educational attainment" "and country of birth (Norway and Sweden)," "household disposable income (Sweden)," "smoking during pregnancy (UK and Sweden)," "maternal BMI (Sweden), number of primary care" "consultations before pregnancy (UK and Norway)", size(*0.35) justification(left) placement(e)) ///
	text(48 0.1 "{sup:‡}Norway and Sweden only", size(*0.35) justification(left) placement(e)) ///
	xsize(90) ysize(100) name(trim3, replace)
	
	* Save graph
	
	graph export "C:\Users\ti19522\OneDrive - University of Bristol\Flo Martin Supervisory Team\Year 4\6_Birth outcomes\supplementary figures\trim3_ma.pdf", replace
...
********************************************************************************
	...
	label define model_lb 1"t1" 2"t2" 3"t3"
	...
	replace _LABELS="Trimester one" if model==1 & _USE==3
	replace _LABELS="Trimester two" if model==2 & _USE==3
	replace _LABELS="Trimester three" if model==3 & _USE==3
	...
	* Macros to create the null line
	...
	(scatter seq _ES if _LABELS=="Trimester one", mcolor("239 121 138") ms(d) msize(small) mlcolor(black) mlw(thin)) ///
	(scatter seq _ES if _LABELS=="Trimester two", mcolor("115 104 59") ms(d) msize(small) mlcolor(black) mlw(thin)) ///
	(scatter seq _ES if _LABELS=="Trimester three", mcolor("97 63 117") ms(d) msize(small) mlcolor(black) mlw(thin)), yscale(reverse) yscale(range(-2.5 34)) ///
	ylab("", angle(0) labsize(*0.6) notick nogrid nogextend) ///
	yline(-3) yline(0) yline(5, lcolor(gray) lpattern(dot)) yline(10, lcolor(gray) lpattern(dot)) yline(15, lcolor(gray) lpattern(dot)) yline(20, lcolor(gray) lpattern(dot)) yline(25, lcolor(gray) lpattern(dot)) yline(30, lcolor(gray) lpattern(dot)) ///
	title("{bf}Pooled odds ratios for antidepressant use by trimester" "{bf}and birth outcomes in the UK, Norway, & Sweden", size(*0.5)) ///
	text(-1.5 2.9 "{bf}aOR* (95% CI)", size(*0.45) justification(left) placement(e)) ///
	text(-1.5 0.05 "{bf}Birth outcome" "Trimester", size(*0.45) justification(left) placement(e)) ///
	text(-1.5 0.16 "{bf}Pooled" "{bf:exposed} {it:n} / N", size(*0.45) justification(right) placement(w)) ///
	text(-1.5 0.38 "{bf}Pooled" "{bf:unexposed} {it:n} / N", size(*0.45) justification(right) placement(w)) ///
	text(38.25 1 "{bf:Pooled aOR (95% CI)}" "from fixed effects meta-analysis", size(*0.4)) ///
	text(38 0.05 "*Adjusted for year of birth, maternal age, previous stillbirth," "parity, antipsychotic and anti-seizure medication use before" "pregnancy, depression, anxiety, practice-level IMD and" "ethnicity (UK), maternal educational attainment and country" "of birth (Norway and Sweden), household disposable income" "(Sweden), smoking during pregnancy (UK and Sweden)," "maternal BMI (Sweden), number of primary care consultations" "before pregnancy (UK and Norway)", size(*0.35) justification(left) placement(e)) ///
	text(41.25 0.05 "{sup:‡}Norway and Sweden only", size(*0.35) justification(left) placement(e)) ///
		text(1 0.05 "{bf:Stillborn}", size(*0.45) justification(left) placement(e)) ///
		text(2  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(2 0.16 "`total_exp_2'", size(*0.45) justification(right) placement(w)) ///
	text(2 0.38 "`total_unexp_2'", size(*0.45) justification(right) placement(w)) ///
	text(2 2.9 "`_ES_2_f' (`_LCI_2_f' – `_UCI_2_f')", size(*0.45) justification(left) placement(e)) ///
		text(3  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(3 0.16 "`total_exp_3'", size(*0.45) justification(right) placement(w)) ///
	text(3 0.38 "`total_unexp_3'", size(*0.45) justification(right) placement(w)) ///
	text(3 2.9 "`_ES_3_f' (`_LCI_3_f' – `_UCI_3_f')", size(*0.45) justification(left) placement(e)) ///
		text(4  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(4 0.16 "`total_exp_4'", size(*0.45) justification(right) placement(w)) ///
	text(4 0.38 "`total_unexp_4'", size(*0.45) justification(right) placement(w)) ///
	text(4 2.9 "`_ES_4_f' (`_LCI_4_f' – `_UCI_4_f')", size(*0.45) justification(left) placement(e)) ///
		text(6  0.05 "{bf:Neonatal death{sup:‡}}", size(*0.45) justification(left) placement(e)) ///
		text(7  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(7 0.16 "`total_exp_7'", size(*0.45) justification(right) placement(w)) ///
	text(7 0.38 "`total_unexp_7'", size(*0.45) justification(right) placement(w)) ///
	text(7 2.9 "`_ES_7_f' (`_LCI_7_f' – `_UCI_7_f')", size(*0.45) justification(left) placement(e)) ///
		text(8  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(8 0.16 "`total_exp_8'", size(*0.45) justification(right) placement(w)) ///
	text(8 0.38 "`total_unexp_8'", size(*0.45) justification(right) placement(w)) ///
	text(8 2.9 "`_ES_8_f' (`_LCI_8_f' – `_UCI_8_f')", size(*0.45) justification(left) placement(e)) ///
		text(9  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(9 0.16 "`total_exp_9'", size(*0.45) justification(right) placement(w)) ///
	text(9 0.38 "`total_unexp_9'", size(*0.45) justification(right) placement(w)) ///
	text(9 2.9 "`_ES_9_f' (`_LCI_9_f' – `_UCI_9_f')", size(*0.45) justification(left) placement(e)) ///
		text(11  0.05 "{bf:Preterm delivery}", size(*0.45) justification(left) placement(e)) ///
		text(12  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(12 0.16 "`total_exp_12'", size(*0.45) justification(right) placement(w)) ///
	text(12 0.38 "`total_unexp_12'", size(*0.45) justification(right) placement(w)) ///
	text(12 2.9 "`_ES_12_f' (`_LCI_12_f' – `_UCI_12_f')", size(*0.45) justification(left) placement(e)) ///
		text(13  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(13 0.16 "`total_exp_13'", size(*0.45) justification(right) placement(w)) ///
	text(13 0.38 "`total_unexp_13'", size(*0.45) justification(right) placement(w)) ///
	text(13 2.9 "`_ES_13_f' (`_LCI_13_f' – `_UCI_13_f')", size(*0.45) justification(left) placement(e)) ///
		text(14  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(14 0.16 "`total_exp_14'", size(*0.45) justification(right) placement(w)) ///
	text(14 0.38 "`total_unexp_14'", size(*0.45) justification(right) placement(w)) ///
	text(14 2.9 "`_ES_14_f' (`_LCI_14_f' – `_UCI_14_f')", size(*0.45) justification(left) placement(e)) ///
		text(16  0.05 "{bf:Post-term delivery}", size(*0.45) justification(left) placement(e)) ///
		text(17  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(17 0.16 "`total_exp_17'", size(*0.45) justification(right) placement(w)) ///
	text(17 0.38 "`total_unexp_17'", size(*0.45) justification(right) placement(w)) ///
	text(17 2.9 "`_ES_17_f' (`_LCI_17_f' – `_UCI_17_f')", size(*0.45) justification(left) placement(e)) ///
		text(18  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(18 0.16 "`total_exp_18'", size(*0.45) justification(right) placement(w)) ///
	text(18 0.38 "`total_unexp_18'", size(*0.45) justification(right) placement(w)) ///
	text(18 2.9 "`_ES_18_f' (`_LCI_18_f' – `_UCI_18_f')", size(*0.45) justification(left) placement(e)) ///
		text(19  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(19 0.16 "`total_exp_19'", size(*0.45) justification(right) placement(w)) ///
	text(19 0.38 "`total_unexp_19'", size(*0.45) justification(right) placement(w)) ///
	text(19 2.9 "`_ES_19_f' (`_LCI_19_f' – `_UCI_19_f')", size(*0.45) justification(left) placement(e)) ///
		text(21  0.05 "{bf:Small for gestational age}", size(*0.45) justification(left) placement(e)) ///
		text(22  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(22 0.16 "`total_exp_22'", size(*0.45) justification(right) placement(w)) ///
	text(22 0.38 "`total_unexp_22'", size(*0.45) justification(right) placement(w)) ///
	text(22 2.9 "`_ES_22_f' (`_LCI_22_f' – `_UCI_22_f')", size(*0.45) justification(left) placement(e)) ///
		text(23  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(23 0.16 "`total_exp_23'", size(*0.45) justification(right) placement(w)) ///
	text(23 0.38 "`total_unexp_23'", size(*0.45) justification(right) placement(w)) ///
	text(23 2.9 "`_ES_23_f' (`_LCI_23_f' – `_UCI_23_f')", size(*0.45) justification(left) placement(e)) ///
		text(24  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(24 0.16 "`total_exp_24'", size(*0.45) justification(right) placement(w)) ///
	text(24 0.38 "`total_unexp_24'", size(*0.45) justification(right) placement(w)) ///
	text(24 2.9 "`_ES_24_f' (`_LCI_24_f' – `_UCI_24_f')", size(*0.45) justification(left) placement(e)) ///
		text(26  0.05 "{bf:Large for gestational age}", size(*0.45) justification(left) placement(e)) ///
		text(27  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(27 0.16 "`total_exp_27'", size(*0.45) justification(right) placement(w)) ///
	text(27 0.38 "`total_unexp_27'", size(*0.45) justification(right) placement(w)) ///
	text(27 2.9 "`_ES_27_f' (`_LCI_27_f' – `_UCI_27_f')", size(*0.45) justification(left) placement(e)) ///
		text(28  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(28 0.16 "`total_exp_28'", size(*0.45) justification(right) placement(w)) ///
	text(28 0.38 "`total_unexp_28'", size(*0.45) justification(right) placement(w)) ///
	text(28 2.9 "`_ES_28_f' (`_LCI_28_f' – `_UCI_28_f')", size(*0.45) justification(left) placement(e)) ///
		text(29  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(29 0.16 "`total_exp_29'", size(*0.45) justification(right) placement(w)) ///
	text(29 0.38 "`total_unexp_29'", size(*0.45) justification(right) placement(w)) ///
	text(29 2.9 "`_ES_29_f' (`_LCI_29_f' – `_UCI_29_f')", size(*0.45) justification(left) placement(e)) ///
		text(31  0.05 "{bf:Low Apgar score{sup:‡}}", size(*0.45) justification(left) placement(e)) ///
		text(32  0.05 "Trimester 1", size(*0.45) justification(left) placement(e)) ///
	text(32 0.16 "`total_exp_32'", size(*0.45) justification(right) placement(w)) ///
	text(32 0.38 "`total_unexp_32'", size(*0.45) justification(right) placement(w)) ///
	text(32 2.9 "`_ES_32_f' (`_LCI_32_f' – `_UCI_32_f')", size(*0.45) justification(left) placement(e)) ///
		text(33  0.05 "Trimester 2", size(*0.45) justification(left) placement(e)) ///
	text(33 0.16 "`total_exp_33'", size(*0.45) justification(right) placement(w)) ///
	text(33 0.38 "`total_unexp_33'", size(*0.45) justification(right) placement(w)) ///
	text(33 2.9 "`_ES_33_f' (`_LCI_33_f' – `_UCI_33_f')", size(*0.45) justification(left) placement(e)) ///
		text(34  0.05 "Trimester 3", size(*0.45) justification(left) placement(e)) ///
	text(34 0.16 "`total_exp_34'", size(*0.45) justification(right) placement(w)) ///
	text(34 0.38 "`total_unexp_34'", size(*0.45) justification(right) placement(w)) ///
	text(34 2.9 "`_ES_34_f' (`_LCI_34_f' – `_UCI_34_f')", size(*0.45) justification(left) placement(e)) ///
	xscale(range(0.05 5.7) log) xlab(0.4(0.2)2.8, labsize(*0.4) format(%3.1fc) angle(45)) xscale(range(0.1 4)) plotregion(margin(1 1 0 1)) yscale(noline) ///
	graphregion(color(white) fcolor(white) ifcolor(white) lcolor(white)) legend(order(3 "T1" 4 "T2" 5 "T3") size(*0.75) pos(5) col(1) region(lcolor(black))) /// 
	xsize(90) ysize(100) name(trim_all, replace) 
	
	graph export "C:\Users\ti19522\OneDrive - University of Bristol\Flo Martin Supervisory Team\Year 4\6_Birth outcomes\ch6_trimester_fig.pdf", replace
		
	/*forvalues x=1/3 {
		
		erase trimesterspec_ma_`x'.dta
