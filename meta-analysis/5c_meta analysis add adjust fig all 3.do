...
* Make the maternal meta-analysis figure for supplementary material and generate the summary effect estimate for the primary figure
	...
	use "$Graphdir\data\add adjust analyses data_all3.dta", clear
	
	gen logor = log(or)
	gen loglci = log(lci)
	gen loguci = log(uci)
	
	metan logor loglci loguci, eform by(outcome) lcols(outcome total_exp total_unexp) saving(addadjust_ma, replace)
	
	* Use the data from metan to make a nicer forest plot
	
	use addadjust_ma, clear
	...
	*drop if _USE==1
	...
	forestplot, eform lcols(total_exp total_unexp) effect("{bf}Odds ratio") null(1) olineopts(lcolor(none)) nlineopts(lcolor(cranberry) lpattern(dash)) xlabel(0.5(0.5)2) title("{bf}Additional adjustment sensitivity analysis", size(small)) ///
	...
	name(addadjust_ma, replace)
	
	cd "$Graphdir"
	graph export addadjust_ma.pdf, replace
	...
	* Macros to create the null line
	...
	title("{bf}Fixed-effect meta-analysis of maternal antidepressant use" "{bf}during pregnancy and birth outcomes in the UK, Norway, & Sweden" "with additional adjustment for dropped covariates", size(*0.5)) ///
	...
	text(44 0.1 "*Adjusted for year of birth, maternal age," "previous stillbirth, parity, antipsychotic and" "anti-seizure medication use before pregnancy," "depression, anxiety, practice-level IMD and" "ethnicity (UK), maternal educational attainment" "and country of birth (Norway and Sweden)," "household disposable income (Sweden)," "smoking during pregnancy," "maternal BMI, number of primary care" "consultations before pregnancy (UK and Norway)", size(*0.35) justification(left) placement(e)) ///
	text(48 0.1 "{sup:‡}Norway and Sweden only", size(*0.35) justification(left) placement(e)) ///
	xsize(90) ysize(100) name(add_adj_sample, replace)
	
	* Save graph
	
	graph export "C:\Users\ti19522\OneDrive - University of Bristol\Flo Martin Supervisory Team\Year 4\6_Birth outcomes\supplementary figures\add_adj_ma.pdf", replace
	
	erase addadjust_ma.dta