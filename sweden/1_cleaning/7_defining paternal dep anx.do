...
* Start logging
	...
	log using "$Logdir\7_defining paternal dep anx", name(defining_paternal_dep_anx) replace
	...
	foreach indic in depression anxiety {
		...
		* Ever
		...
		forvalues n=1/`max' {
			...
			use "$Deriveddir\preg_cohort_pat.dta", clear
			keep father_id start_date deliv_date pregnum_dad
			
			keep if pregnum_dad==`n'
			
			merge 1:m father_id using "$Datadir\indications\\`indic'_paternal_compress.dta", keep(3) nogen
			...
			if _N>0 {
				...
				keep father_id start_date deliv_date `indic'_ever count_`indic'_ever
				duplicates drop
		...
		* 12 months pre-pregnancy
		...
		forvalues n=1/`max' {
			...
			if _N>0 {
				...
				keep father_id start_date deliv_date `indic'_12mo count_`indic'_12mo
				duplicates drop
		...
		* During pregnancy
		...
		forvalues n=1/`max' {
			...
			use "$Deriveddir\preg_cohort_pat.dta", clear
			keep father_id start_date deliv_date pregnum_dad deliv_date
			...
			if _N>0 {
				...
				keep father_id start_date deliv_date `indic'_preg count_`indic'_preg
				duplicates drop
		...
		* Create depression and anxiety dataset
		...
		merge 1:1 father_id start_date deliv_date using "$Tempdatadir\12mo_prepreg_`indic'.dta", nogen
		merge 1:1 father_id start_date deliv_date using "$Tempdatadir\preg_`indic'.dta", nogen
...
* Stop logging, translate .smcl into .pdf and erase .smcl
	...
	translate "$Logdir\7_defining paternal dep anx.smcl" "$Logdir\7_defining paternal dep anx.pdf", replace
	
	erase "$Logdir\7_defining paternal dep anx.smcl"
