...
********************************************************************************
	...
	* Load in the pregnancy dataset
	...
	tab pregnum_dad // n=31 pregnancies max
	summ pregnum_dad
	local max=`r(max)'
	...
	* Merge on to prescription information and define exposure information for each pregnancy a mother has
	...
	/*forvalues x=28/31 {
		...
		* Pregnancy period
		...
		foreach y in a b c { // pregnancy
			...
			keep father_id start_date deliv_date disp_date prescr_end_date drugsubstance drugsubstance_num class_pdr
			...
			if _N>0 { // some datasets with no observations
				...
				* Reshape data to one row per person
				reshape wide drugsubstance drugsubstance_num class_pdr disp_date prescr_end_date, i(father_id deliv_date start_date) j(_presseq)
				...
				keep father_id deliv_date start_date
	...
	bysort deliv_date (father_id): keep if pregnum_dad==`x'
		...
		foreach y in a b c { // pregnancy
			...
			merge 1:1 father_id deliv_date using "$Tempdatadir\preg_`x'_period`y'.dta", keep(1 3) 
	...
	* Merge onto full pregnancy cohort to check - all matched
	merge 1:1 father_id start_date deliv_date using "$Deriveddir\preg_cohort_pat.dta", nogen 

	keep father_id deliv_date pregnum_dad *a *b *c
