...
* Script to be called on during the child data cleaning script for externally validating birthweight against the Intergrowth-21 standards using the Swedish data
...
* Load in the data 
	...
	keep mother_id child_id deliv_date birth_weight child_male gest_age_days
	...
	* Create variable 
	...
	foreach sex in 0 1 {
	    forvalues week = 168(1)230 {
		   ...
		   replace sga_vpt_ex = 1 if gest_age_days==(`week') & birweit_kg<(`cut_`sex'_`week'_10_vpt') & child_male==(`sex')
		   replace lga_vpt_ex = 1 if gest_age_days==(`week') & birweit_kg>(`cut_`sex'_`week'_90_vpt') & child_male==(`sex')
	...
	foreach sex in 0 1 {
	    forvalues week = 231(1)300 {
		   ...
		   replace sga_ex = 1 if gest_age_days==(`week') & birweit_kg<(`cut_`sex'_`week'_10') & child_male==(`sex')
		   replace lga_ex = 1 if gest_age_days==(`week') & birweit_kg>(`cut_`sex'_`week'_90') & child_male==(`sex')
	...
	keep mother_id child_id deliv_date sga_ex lga_ex nga_ex
	
	rename sga_ex sga_ex_intergrowth
	rename nga_ex nga_ex_intergrowth
