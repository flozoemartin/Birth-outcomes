...
* Start logging
	...
	log using "$Logdir\5_defining maternal indication", name(defining_maternal_indication) replace
	...
	foreach indic in depression anxiety bipolar ed migraine incont headache {
		...
		* Ever
		...
		forvalues n=1/`max' {
			...
			use "$Deriveddir\preg_cohort.dta", clear
			keep mother_id deliv_date start_date pregnum
			
			keep if pregnum==`n'
			
			merge 1:m mother_id using "$Datadir\indications\\`indic'_maternal_compress.dta", keep(3) nogen
			...
			if _N>0 {
				...
				keep mother_id deliv_date `indic'_ever count_`indic'_ever
				duplicates drop
		...
		* 12 months pre-pregnancy
		...
		forvalues n=1/`max' {
			...
			if _N>0 {
				...
				keep mother_id deliv_date `indic'_12mo count_`indic'_12mo
				duplicates drop
		...
		* During pregnancy
		...
		forvalues n=1/`max' {
			...
			use "$Deriveddir\preg_cohort.dta", clear
			keep mother_id deliv_date start_date pregnum deliv_date
			...
			if _N>0 {
				...
				keep mother_id deliv_date `indic'_preg count_`indic'_preg
				duplicates drop
		...
		* Create indication datasets
		...
		merge 1:1 mother_id deliv_date using "$Tempdatadir\12mo_prepreg_`indic'.dta", nogen
		merge 1:1 mother_id deliv_date using "$Tempdatadir\preg_`indic'.dta", nogen
...
* Stop logging, translate .smcl into .pdf and erase .smcl
	...
	translate "$Logdir\5_defining maternal indication.smcl" "$Logdir\5_defining maternal indication.pdf", replace
	
	erase "$Logdir\5_defining maternal indication.smcl"
